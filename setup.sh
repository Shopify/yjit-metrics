...
setup-repos () {
  ...
  [[ -d raw-benchmark-data ]] || git clone --single-branch --branch main https://github.com/rubybench/benchmark-data raw-benchmark-data
  # Clone github pages repo for pushing built reports.
  [[ -d ghpages-yjit-metrics ]] || git clone --single-branch --branch pages https://github.com/rubybench/yjit-reports ghpages-yjit-metrics
}
